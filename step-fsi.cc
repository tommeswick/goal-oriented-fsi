/**
 ...
 T. Richter, T. Wick; Variational Localizations of the Dual-Weighted Residual Estimator,
 ...
 1. Construct symmetric error estimator with primal \rho and adjoint \rho^* error parts
    (see Becker/Rannacher, Acta Numerica, 2001
     or Bangerth/Rannacher, Book, 2003)
 ...
 4. Better (cheaper) approximation of dual solution via 
    local-higher order interpolations rather than a global higher-order finite element
...
private:
  ...
  void assemble_rhs_adjoint_drag ();
  void assemble_rhs_adjoint_pressure_point ();
  void assemble_rhs_adjoint_displacement_point ();
  void set_bc_adjoint ();
  ...
  double TOL_DWR_estimator, lower_bound_newton_residual,lower_bound_newton_residual_relative; 

  unsigned int refinement_strategy;
  
  double reference_value_drag, reference_value_lift, reference_value_p_front, 
    reference_value_p_diff, exact_error_local, reference_value_flag_tip_ux, reference_value_flag_tip_uy;
...
FSI_PU_DWR_Problem<dim>::FSI_PU_DWR_Problem (const unsigned int degree)
		...
		triangulation (Triangulation<dim>::none),
                ...
                fe_primal (FE_Q<dim>(2), dim,  // velocities                  
		...
		fe_adjoint (FE_Q<dim>(4), dim,  // velocities                  
			    FE_Q<dim>(4), dim,  // displacements		    
			    FE_Q<dim>(2), 1),   // pressure
...
// other results. 
template <int dim>
void FSI_PU_DWR_Problem<dim>::set_runtime_parameters ()
{
  // Defining test cases
...
template <int dim>
void FSI_PU_DWR_Problem<dim>::setup_system_primal ()
{
...
template <int dim>
void FSI_PU_DWR_Problem<dim>::assemble_matrix_primal ()
{
      ...
      if (cell->material_id() == 0)
	{
	  for (unsigned int q=0; q<n_q_points; ++q)
		...
		{	
		  ...
		  for (unsigned int j=0; j<dofs_per_cell; ++j)
			...
			{		
			  local_matrix(j,i) += (convection_fluid_LinAll_short * phi_i_v[j] +
						scalar_product(stress_fluid_ALE_1st_term_LinAll, phi_i_grads_v[j]) +
			...
			{
			  // Harmonic MMPDE
			  local_matrix(j,i) += (alpha_u * scalar_product(phi_i_grads_u[i], phi_i_grads_u[j])
						) * fe_values.JxW(q);
	  ...
	  for (unsigned int face=0; face<GeometryInfo<dim>::faces_per_cell; ++face)
		  ...
		  )
		{
		  ...
		  fe_face_values.get_function_values (solution_primal, old_solution_face_values);
		  fe_face_values.get_function_gradients (solution_primal, old_solution_face_grads);	
		      ...
		      for (unsigned int i=0; i<dofs_per_cell; ++i)
			{
			  ...
			  for (unsigned int j=0; j<dofs_per_cell; ++j)
			    {		     
			      ...
			      if (comp_j == 0 || comp_j == 1)
				{
				  local_matrix(j,i) -= neumann_value * phi_i_v[j] 
	  ...
	  for (unsigned int q=0; q<n_q_points; ++q)
		...
		{	    	     		
		  ...
		  for (unsigned int j=0; j<dofs_per_cell; ++j)
			...
			{
			  local_matrix(j,i) += density_structure * phi_i_v[i] * phi_i_u[j]				
			    *  fe_values.JxW(q);			  
			}
		      else if (comp_j == 4)
			{
	      ...
	      for (unsigned int i=0; i<dofs_per_cell; ++i)
		{
		  ...
		  if (comp_i == 0 || comp_i == 1)
		    ...
		    {	
		      ...
		      local_rhs(i) -= (alpha_u * scalar_product(grad_u, phi_i_grads_u)
				       ) * fe_values.JxW(q);
		  ...
		  )
		{
		  ...
		  for (unsigned int q=0; q<n_face_q_points; ++q)
		      ...
		      for (unsigned int i=0; i<dofs_per_cell; ++i)
			{
			  ...
			  if (comp_i == 0 || comp_i == 1)
			    {  
			      local_rhs(i) +=  neumann_value * fe_face_values[velocities].value (i, q) 	     
				* fe_face_values.JxW(q);					   
			    }
			  // end i
			}  
	      ...
	      for (unsigned int i=0; i<dofs_per_cell; ++i)
		{
		  ...
		  if (comp_i == 0 || comp_i == 1)
		  ...
		  else if (comp_i == 2 || comp_i == 3)
		    {
		      ...
		      local_rhs(i) -=  density_structure *  v * phi_i_u
			* fe_values.JxW(q);    
		      
		    }
		  else if (comp_i == 4)
		    {
		      // Laplace continuation pressure. Explanation see in the assemble_matrix part.
...
template <int dim>
void FSI_PU_DWR_Problem<dim>::newton_iteration_primal () 
...
{ 
  ...
  while (newton_residual > lower_bound_newton_residual &&
	 newton_residual/initial_residual > lower_bound_newton_residual_relative &&
	...
	{	     					 
      ...
      std::cout << std::setprecision(5) << newton_step << '\t' 
		...
		<< std::scientific << newton_residual/initial_residual << '\t'
		<< std::scientific << std::abs(solution_norm - old_solution_norm) << '\t'
		<< std::scientific << std::abs(solution_norm - old_solution_norm)/old_solution_norm << '\t'
  ...
  std::cout << "Wall time solving primal system:  " << timer_newton_global() << std::endl;
...
// This function is similar to many deal.II tuturial steps.
template <int dim>
void FSI_PU_DWR_Problem<dim>::setup_system_adjoint ()
{
...
template <int dim>
void FSI_PU_DWR_Problem<dim>::assemble_matrix_adjoint ()
{
 ...
 FEFaceValues<dim> fe_face_values_primal (fe_primal, face_quadrature_formula, 
				   update_values         | update_quadrature_points  |
				    update_normal_vectors | update_gradients |
				    update_JxW_values);
 ...
 std::vector<Vector<double> >  old_solution_face_values_primal (n_face_q_points, 
							  Vector<double>(dim+dim+1));
       
  std::vector<std::vector<Tensor<1,dim> > > old_solution_face_grads_primal (n_face_q_points, 
								     std::vector<Tensor<1,dim> > (dim+dim+1));
      ...
      if (cell->material_id() == 0)
	{
	  for (unsigned int q=0; q<n_q_points; ++q)
	      ...
	      for (unsigned int k=0; k<dofs_per_cell; ++k)
		{
	      ...
	      const Tensor<2,dim> pI = ALE_Transformations		
		::get_pI<dim> (q, old_solution_values_primal);
	      
	      const Tensor<1,dim> v = ALE_Transformations
	      	::get_v<dim> (q, old_solution_values_primal);
	      
	      const Tensor<2,dim> grad_v = ALE_Transformations
		::get_grad_v<dim> (q, old_solution_grads_primal);	
	      
	      const Tensor<2,dim> grad_v_T = ALE_Transformations
		::get_grad_v_T<dim> (grad_v);

	      const Tensor<2,dim> F = ALE_Transformations
		::get_F<dim> (q, old_solution_grads_primal);	    
		...
		{	
		  ...
		  const double J_LinU =  ALE_Transformations
		    ::get_J_LinU<dim> (q, old_solution_grads_primal, phi_i_grads_u[i]);
		  
		  const Tensor<2,dim> J_F_Inverse_T_LinU = ALE_Transformations
		    ::get_J_F_Inverse_T_LinU<dim> (phi_i_grads_u[i]);
		  
		  const Tensor<2,dim> F_Inverse_LinU = ALE_Transformations
		    ::get_F_Inverse_LinU (phi_i_grads_u[i], J, J_LinU, q, old_solution_grads_primal);
		  ...
		  const double incompressibility_ALE_LinAll = NSE_in_ALE
		    ::get_Incompressibility_ALE_LinAll<dim> 
		    (phi_i_grads_v[i], phi_i_grads_u[i], q, old_solution_grads_primal); 
		  ...
		  for (unsigned int j=0; j<dofs_per_cell; ++j)
		    {	
		      ...
		      if (comp_j == 0 || comp_j == 1)
			{
			  local_matrix(i,j) += (convection_fluid_LinAll_short * phi_i_v[j] +
						scalar_product(stress_fluid_ALE_1st_term_LinAll, phi_i_grads_v[j]) +		
						scalar_product(stress_fluid_ALE_2nd_term_LinAll, phi_i_grads_v[j]) 	
						) * fe_values.JxW(q);
			}					    
			...
			{
			  // Linear harmonic MMPDE
			  local_matrix(i,j) += (alpha_u * scalar_product(phi_i_grads_u[i], phi_i_grads_u[j])
						) * fe_values.JxW(q);

			}
		      else if (comp_j == 4)
			{
			  local_matrix(i,j) += (incompressibility_ALE_LinAll *  phi_i_p[j] 
						) * fe_values.JxW(q);		
			}
	  ...
	  for (unsigned int face=0; face<GeometryInfo<dim>::faces_per_cell; ++face)
		  ...
		  )
		{
		  ...
		  fe_face_values.get_function_values (solution_adjoint, old_solution_face_values);
		  fe_face_values.get_function_gradients (solution_adjoint, old_solution_face_grads);

		  // Old primal Newton iteration values
		  fe_face_values_primal.get_function_values (solution_primal, old_solution_face_values_primal);
		  fe_face_values_primal.get_function_gradients (solution_primal, old_solution_face_grads_primal);	
		  ...
		  for (unsigned int q=0; q<n_face_q_points; ++q)
		      ...
		      for (unsigned int k=0; k<dofs_per_cell; ++k)
			{
		      ...
		      const Tensor<2,dim>  grad_v = ALE_Transformations
			::get_grad_v<dim> (q, old_solution_face_grads_primal);
		      
		      const Tensor<2,dim> F = ALE_Transformations
			::get_F<dim> (q, old_solution_face_grads_primal);
		      ...
		      for (unsigned int i=0; i<dofs_per_cell; ++i)
			{
			  ...
			  const double J_LinU = ALE_Transformations
			    ::get_J_LinU<dim> (q, old_solution_face_grads_primal, phi_i_grads_u[i]);
			  ...
			  const Tensor<2,dim> F_Inverse_LinU = ALE_Transformations
			    ::get_F_Inverse_LinU 
			    (phi_i_grads_u[i], J, J_LinU, q, old_solution_face_grads_primal);
			  ...
			  for (unsigned int j=0; j<dofs_per_cell; ++j)
			      ...
			      if (comp_j == 0 || comp_j == 1)
				{
				  ...
				  local_matrix(i,j) -= neumann_value * phi_i_v[j] 
				    * fe_face_values.JxW(q);
		  ...
		  for (unsigned int j=0; j<dofs_per_cell; ++j)
		    {
		      ...
		      if (comp_j == 0 || comp_j == 1)
			{
			  local_matrix(i,j) += (scalar_product(piola_kirchhoff_stress_structure_STVK_LinALL, 
							       phi_i_grads_v[j]) 
						) * fe_values.JxW(q);      	
			}		     
		      else if (comp_j == 2 || comp_j == 3)
			{
			  local_matrix(i,j) += (density_structure * phi_i_v[i] * phi_i_u[j]
						) *  fe_values.JxW(q);			  
			}
		      else if (comp_j == 4)
			{
			  local_matrix(i,j) += alpha_u * (phi_i_grads_p[i] * phi_i_grads_p[j]) * fe_values.JxW(q);      
			}
		      // end j dofs
		    }  
		  // end i dofs		     
		  ...
		  )
		{
		  ...
		  for (unsigned int q=0; q<n_face_q_points; ++q)
		    {	
		      ...
		      for (unsigned int i=0; i<dofs_per_cell; ++i)
			{
			  const Tensor<2,dim> phi_i_grads_v = fe_face_values[velocities].gradient (i, q);
  ...
  for (; cell!=endc; ++cell)
	...
	if (cell->vertex(vertex).distance(evaluation_point)
	    < cell->diameter()*1e-8)
	  {
	    ...
	    // The second argument is the FE component: 0 (vx), 1(vy), 4(pressure)
		 ...
		 < cell->diameter()*1e-8)
	  {
	    ...
	    // The first argument is are the vertex coordinates
	    // The second argument is the FE component: 0 (vx), 1(vy), 4(pressure)
	    // The minus is there because the goal functional is a pressure difference
	    system_rhs_adjoint(cell->vertex_dof_index(vertex,4)) = -1;
	  }
...
FSI_PU_DWR_Problem<dim>::solve_adjoint () 
{
  ...
  if (adjoint_rhs == "drag")
    ...
    assemble_rhs_adjoint_drag (); // drag and lift are assembled in the same function
  ...
  std::cout << "Wall time solving adjoint system: " << timer_solve_adjoint() << std::endl;
...
template <int dim>
void
FSI_PU_DWR_Problem<dim>::output_results (const unsigned int refinement_cycle)  const
{
...

 {
  ...
  std::vector<DataComponentInterpretation::DataComponentInterpretation>
    ...
    (dim+dim+1+dim+dim+1, DataComponentInterpretation::component_is_scalar);
...
template <int dim>
double FSI_PU_DWR_Problem<dim>::compute_point_value (Point<dim> p, 
					       const unsigned int component) const  
{
...
template <int dim>
void FSI_PU_DWR_Problem<dim>::compute_drag_lift_fsi_fluid_tensor()
{
...
template <int dim>
void FSI_PU_DWR_Problem<dim>::compute_drag_lift_fsi_fluid_tensor_domain()
{
...
template <int dim>
void FSI_PU_DWR_Problem<dim>::compute_drag_lift_fsi_fluid_tensor_domain_structure()
{
...
template <int dim>
void FSI_PU_DWR_Problem<dim>::compute_minimal_J()
{
  QGauss<dim>   quadrature_formula(degree+2);
  FEValues<dim> fe_values (fe_primal, quadrature_formula,
                           update_values    |
...
template<int dim>
void FSI_PU_DWR_Problem<dim>::compute_functional_values()
{
  double x1,y1, p_front, p_back, p_diff;
  x1 = compute_point_value(Point<dim>(0.6,0.2), dim); // dis-x of flag tip (FSI 1)
  y1 = compute_point_value(Point<dim>(0.6,0.2), dim+1); // dis-y of flag tip (FSI 1)
  
  p_front = compute_point_value(Point<dim>(0.15,0.2), dim+dim); // pressure
  p_back  = 0.0; //compute_point_value(Point<dim>(0.25,0.2), dim+dim); // pressure
...
template<int dim>
void FSI_PU_DWR_Problem<dim>::refine_mesh()
{
  typename DoFHandler<dim>::active_cell_iterator
    cell = dof_handler_primal.begin_active(),
    endc = dof_handler_primal.end();
...
template<int dim>
double FSI_PU_DWR_Problem<dim>::compute_error_indicators_a_la_PU_DWR (const unsigned int refinement_cycle)
{
  ...
  const FEValuesExtractors::Vector displacements (dim); // 2
   ...
   for ( ; cell!=endc; ++cell, ++cell_adjoint)
       ...
       for (unsigned int q=0; q<n_q_points; ++q)
	 {
	  ...
	  const double incompressiblity_fluid = NSE_in_ALE
	    ::get_Incompressibility_ALE<dim> (q, primal_cell_gradients);
	  ...
	  for (unsigned int i=0; i<dofs_per_cell; ++i)
	    {
	      ...
	      grad_phi_psi_v[0][0] = fe_values_pou[pou_extract].value(i,q) * grad_dw_v[0][0] + dw_v[0] * fe_values_pou[pou_extract].gradient(i,q)[0];
	      grad_phi_psi_v[0][1] = fe_values_pou[pou_extract].value(i,q) * grad_dw_v[0][1] + dw_v[0] * fe_values_pou[pou_extract].gradient(i,q)[1];
	      grad_phi_psi_v[1][0] = fe_values_pou[pou_extract].value(i,q) * grad_dw_v[1][0] + dw_v[1] * fe_values_pou[pou_extract].gradient(i,q)[0];
	      grad_phi_psi_v[1][1] = fe_values_pou[pou_extract].value(i,q) * grad_dw_v[1][1] + dw_v[1] * fe_values_pou[pou_extract].gradient(i,q)[1];
		...
		// Therefore numerically no influence
	      ...
	      */
	      ...
	      //double divergence_phi_psi = grad_dw_v[0][0] * cell_data.fe_values_pou[pou_extract].value(i,q) + dw_v[0] * cell_data.fe_values_pou[pou_extract].gradient(i,q)[0]
	      //	+ grad_dw_v[1][1] * cell_data.fe_values_pou[pou_extract].value(i,q) + dw_v[1] * cell_data.fe_values_pou[pou_extract].gradient(i,q)[1];
	      ...
	      if (cell->material_id() == 0)
		{
		  ...
		  // Second part: - (\nabla u, ...)
		  local_err_ind(i) -= (convection_fluid * dw_v * fe_values_pou[pou_extract].value(i,q)
				       + scalar_product(fluid_pressure + stress_fluid, grad_phi_psi_v) 
				       + incompressiblity_fluid  * dw_p * fe_values_pou[pou_extract].value(i,q)
				       ) * fe_values_pou.JxW (q);

		}
	      else if (cell->material_id() == 1)
		{
		  local_err_ind(i) -= (scalar_product(stress_term_solid, grad_phi_psi_v) 
				       ) * fe_values_pou.JxW (q);
...
template<int dim>
double FSI_PU_DWR_Problem<dim>::refine_average_with_PU_DWR (const unsigned int refinement_cycle)
{
  // Step 1:
...
template <int dim>
void FSI_PU_DWR_Problem<dim>::run () 
{  
  ...
  for (unsigned int cycle=0; cycle<max_no_refinement_cycles; ++cycle)
      ...
      if (cycle >= 0)
	{
	  ...
	  if (refinement_strategy == 0)
	    ...
	    {
	      std::cout << "No such refinement strategy. Aborting." << std::endl;
	      abort();
	    }
